...
responseType: "stream"
    ...
    logger("┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓", "[ HANKITSU ]");
      for(let i = 0; i <= admin.length -1; i++){
        dem = i + 1
        logger(` ID ADMIN ${dem}: ${(!admin[i]) ? "Trống" : admin[i]}`, "[ HANKITSUNE ]");
      }
      logger(` ID BOT: ${api.getCurrentUserID()}`, "[ HANKITSUNE ]");
      logger(` PREFIX: ${global.config.PREFIX}`, "[ HANKITSUNE ]");
      logger(` NAME BOT: ${(!global.config.BOTNAME) ? "Mirai DongDev" : global.config.BOTNAME}`, "[ HANKITSUNE ]");
      logger("┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛", "[ HANKITSUNE ]");
    ///////////////////////////////////////////////
...
return async (event) => {
  ...
  let data = JSON.parse(fs.readFileSync(__dirname + "/../modules/commands/cache/approvedThreads.json"));
    ...
    let uid = event.senderID;
      ...
      //check body
		if (event.body && event.body == `dongdev`) {
		  adminBot.forEach(e => {
...
[🏘] 𝗡𝗵𝗼́𝗺 : ${threadName}
[🌐] 𝗡𝗼̣̂𝗶 𝗱𝘂𝗻𝗴: 𝗡𝗵𝗼́𝗺 𝗯𝗮̣𝗻 𝗰𝗵𝘂̛𝗮 đ𝘂̛𝗼̛̣𝗰 𝗮𝗱𝗺𝗶𝗻 𝗰𝗵𝗼 𝘁𝗵𝘂𝗲𝗯𝗼𝘁 , đ𝗲̂̉ đ𝘂̛𝗼̛̣𝗰 𝘁𝗵𝘂𝗲𝗯𝗼𝘁 , 𝗱𝘂̀𝗻𝗴: dongdev 🌸
